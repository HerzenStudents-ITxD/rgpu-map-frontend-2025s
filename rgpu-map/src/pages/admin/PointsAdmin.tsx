  ...
  return (
    <Container maxWidth="md" sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        <Typography variant="h5">{t('admin.pointsManagement')}</Typography>
        <Button component={Link} to="/admin/users" variant="contained">
          {t('admin.usersManagement')}
        </Button>
      </Box>

      <Box sx={{ backgroundColor: 'background.paper', p: 3, borderRadius: 2, boxShadow: 1, mb: 3 }}>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2 }}>
          ...
          <Button variant="contained" onClick={handleAddPoint} sx={{ height: 56 }}>
            {t('admin.addPoint')}
          </Button>
        </Box>
      </Box>
        ...
        <List>
          {points.map((point) => (
            <ListItem
              ...
              secondaryAction={
                <>
                  <IconButton onClick={() => setEditingPoint(point)} color="primary" sx={{ mr: 1 }}>
                    <Edit />
                  </IconButton>
                  <IconButton onClick={() => handleDeletePoint(point.point_id)} color="error">
                    <Delete />
                  </IconButton>
                </>
              }
            ...
            >
              <ListItemText
                ...
                secondary={
                  <>
                    <Box>
                      {t('admin.coordinates')}: X={point.x}, Y={point.y}, Z={point.z}
                    </Box>
                    <Box sx={{ mt: 1 }}>
                      {t('admin.type')}: {{
                        1: t('admin.classroom'),
                    ...
                    {point.connections && point.connections.length > 0 && (
                      <Box sx={{ mt: 1 }}>
                        ...
                        {point.connections.map((id) => (
                          <Chip key={id} label={id} sx={{ mr: 1, mt: 1 }} />
                        ))}
                      </Box>
                    )}
                  </>
        ...
        >
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
              <Button variant="contained" onClick={handleSavePoint} fullWidth>
                {t('admin.saveChanges')}
              </Button>
              <Button variant="outlined" onClick={() => setEditingPoint(null)} fullWidth>
                {t('admin.cancel')}
              </Button>
            </Box>
          </Box>
